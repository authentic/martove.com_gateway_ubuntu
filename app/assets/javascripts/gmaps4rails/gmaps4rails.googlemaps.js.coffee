...
class @Gmaps4RailsGoogle extends Gmaps4Rails
  ...
  constructor: ->
    ...
    @direction_conf =
      ...
      destination: null
      waypoints:
        []       #[{location: "toulouse,fr", stopover: true}, {location: "Clermont-Ferrand, fr", stopover: true}]
  ...
  createMarker: (args) ->
    ...
    markerImage = @createOrRetrieveImage(args.marker_picture, args.marker_width, args.marker_height,
                                         imageAnchorPosition)
    shadowImage = @createOrRetrieveImage(args.shadow_picture, args.shadow_width, args.shadow_height,
                                         shadowAnchorPosition)
  ...
  createOrRetrieveImage: (currentMarkerPicture, markerWidth, markerHeight, imageAnchorPosition) ->
    ...
    switch test_image_index
      when false
        markerImage = @createMarkerImage(currentMarkerPicture, @createSize(markerWidth, markerHeight), null,
                                         imageAnchorPosition, null)
  ...
  createClusterer: (markers_array) ->
    return new MarkerClusterer(@serviceObject, markers_array,
                               {  maxZoom: @markers_conf.clusterer_maxZoom, gridSize: @markers_conf.clusterer_gridSize, styles: @customClusterer() })
  ...
  createInfoWindow: (marker_container) ->
    if typeof(@jsTemplate) == "function" or marker_container.description?
      ...
      if @markers_conf.custom_infowindow_class != null
        ...
        boxText.setAttribute("class", @markers_conf.custom_infowindow_class)
        #to customize
        boxText.innerHTML = marker_container.description
        marker_container.infowindow = new InfoBox(@infobox(boxText))
        currentMap = this
        google.maps.event.addListener(marker_container.serviceObject, 'click',
                                      @openInfoWindow(currentMap, marker_container.infowindow,
                                                      marker_container.serviceObject))
      else
